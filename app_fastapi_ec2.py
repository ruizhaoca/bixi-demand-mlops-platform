"""FastAPI-backed Streamlit app for BIXI 15-minute demand prediction.

Run on EC2:
    streamlit run app_fastapi_ec2.py --server.address=0.0.0.0 --server.port=8501

Deployment mode:
    EC2 Streamlit UI + AWS App Runner FastAPI + S3 artifacts.

The UI never loads model artifacts or calls S3. Every model-dependent operation
is sent to the FastAPI service configured by ``BIXI_API_URL``.
"""
...
from bixi.fastapi_client import (  # noqa: E402
    ApiClientError,
    FastApiClient,
    TARGET_LABELS,
    common_stations,
...
@st.cache_resource(show_spinner="Connecting to the FastAPI prediction service...")
def cached_api_client() -> FastApiClient:
    return FastApiClient.from_env()


@st.cache_resource(show_spinner="Loading stations and monitoring metadata from FastAPI...")
def cached_bundles():
    return load_api_bundles(cached_api_client())
...
def render_page_7_day_prediction(bundles) -> None:
    ...
    st.write(
        "Predict 15-minute BIXI station demand through FastAPI using cached Open-Meteo weather."
    )
...
def render_monitoring(bundles) -> None:
    ...
    st.write("Model metrics, explainability, fairness, and drift metadata served by FastAPI.")
...
@st.cache_data(ttl=3600, show_spinner="Loading rebalancing priorities from FastAPI...")
def cached_rebalancing(_client: FastApiClient, dayofweek: int) -> pd.DataFrame:
    _, risk_df = _client.rebalancing(dayofweek)
    return risk_df


@st.cache_data(ttl=3600, show_spinner="Loading station trajectory from FastAPI...")
def cached_trajectory(
    _client: FastApiClient,
    dayofweek: int,
    station_name: str,
) -> pd.DataFrame:
    trajectory, _ = _client.rebalancing(dayofweek, station_name=station_name)
    return trajectory
...
def render_rebalancing(bundles) -> None:
    ...
    client = next(iter(bundles.values())).client
    risk_df = cached_rebalancing(client, WEEKDAYS[weekday])
    ...
    netflow_df = cached_trajectory(client, WEEKDAYS[weekday], station_name)
...
def render_sidebar(bundles) -> str:
    ...
    st.sidebar.caption("Mode: EC2 Streamlit + App Runner FastAPI")
    st.sidebar.caption(f"API: `{cached_api_client().base_url}`")
...
def main() -> None:
    ...
    try:
        ...
        st.error(f"FastAPI service is unavailable: {exc}")
        st.info("Check BIXI_API_URL, the App Runner service health, and the optional API key.")
        st.stop()
    ...
    try:
        ...
        st.info("The packaged Community Cloud deployment remains available as the fallback UI.")
